...
const SharedChart: FC = () => {
  const router = useRouter();
  const { chart_id, shr } = router.query;
  if (!chart_id && !shr) {
    return <PageLoader />;
  }
  ...
  const [chartState, setChartState] = React.useState<any>({ lineWeight: 1 });
  ...
  React.useEffect(() => {
    setRedraw(!reDraw);
  }, [verticalBarData, verticalStackData, mixedData, areaData, scatterData, bubbleData, radarData]);
  ...
  const getSharedChart = async (token: any) => {
    charts
  ...
  const renderChart = (
    ...
    const filters1 = [...filters.dropdown, ...filters.textBox.filter((el:any) => el.filterInput !== ''), ...filters.slider];
