...
const Slider: React.FC<Props> = (props) => {
  ...
  // const [knobX, updateKnobX] = React.useState<any>(defaultMinBudget);
  ...
  return (
    <Holder className="chart-filters">
      ...
      >
        <AnimatedKnob
          ...
          onDrag={(event, info) => {
            ...
            const newValue = info.point.x > knobXSecond ? knobXSecond : info.point.x;
      ...
      {/* <AmountLabels>
        <small className="pe-4">{name}</small>
        <small
          css={css`
        ...
        >
          {knobX > 0
            ? Math.ceil(parseInt((min / (trackWidthInPx / knobX)).toFixed(0), 10)) * 1
            : "0"}
        </small>
        {"-"}
        <small
          ...
          `}
        >
          {Math.ceil(parseInt((max / (trackWidthInPx / knobXSecond)).toFixed(0), 10)) * 1}
        </small>
      </AmountLabels> */}
      {/* <p
        onClick={() => {
          console.log(max, "max");
          console.log(defaultMinBudget, "min");
          console.log(defaultMaxBudget, "max");
          console.log(knobX, "left");
          console.log(knobXSecond, "right");
          console.log(trackWidth, "track width");
          console.log(trackWidthInPx, " track width in px");
          console.log(trackLength, "track length");
        }}
        className="border"
      >
        Hello
